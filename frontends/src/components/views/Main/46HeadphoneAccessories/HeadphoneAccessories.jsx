...
import './headphoneaccessories.css'

function HeadphoneAccessories() {

  const [Products, setProducts] = useState([])
  const [toggleBrands, settoggleBrands] = useState(1)
  const [UserData, setUserData] = useState([])
  const [ChargingDock, setChargingDock] = useState([])
  const [AudioCable, setAudioCable] = useState([])
  const [SplitterCable, setSplitterCable] = useState([])
  const [ChargingDockValue, setChargingDockValue] = useState("Charging Dock")
  const [AudioCableValue, setAudioCableValue] = useState("Auxilary Cable")
  const [SplitterCableValue, setSplitterCableValue] = useState("Audio Splitter")
  ...
  if (response.data.success) {
    ...
    setChargingDock(response.data.products)
  } else {
  }
  })
...
.then(response => {
  ...
  if (response.data.success) {
    ...
    setAudioCable(response.data.products)
  } else {
  }
  })
  ...
  if (response.data.success) {
    ...
    setSplitterCable(response.data.products)
  } else {
  }
  })
  ...
  const SearchedProducts = Products.map((product, index) => {
    ...
    const ChargingDockPage = ChargingDock.map((product, index) => {
      return (
      ...
      const AudioCablePage = AudioCable.map((product, index) => {
        return (
          <div key={index} className='listing-Item'>
          <div className='listing-Details'>
              <a href={`/product/${product._id}`}> <div className='Item-image'>
      ...
      const SplitterCablePage = SplitterCable.map((product, index) => {
        return (
          <div key={index} className='listing-Item'>
          <div className='listing-Details'>
              <a href={`/product/${product._id}`}> <div className='Item-image'>
...
export default HeadphoneAccessories