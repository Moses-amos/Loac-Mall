...
import './ipodmpthree.css'

function IpodMpthree() {
    ...
    const [IpodMp, setIpodMp] = useState([])
    const [IpodMpValue, setIpodMpValue] = useState("Audio Player")
    ...
    const popUpLogin = (index) => {
      ...
      if (response.data.success) {
        ...
        setIpodMp(response.data.products)
      } else {
      }
      })
    ...
    .then(response => {
      ...
      const SearchedProducts = Products.map((product, index) => {
        ...
        const IpodPage = IpodMp.map((product, index) => {
            if (product.seriesName === "Ipod") {
                return (
            ...
            const MpThreePage = IpodMp.map((product, index) => {
                if (product.seriesName === "Mp3") {
                    return (
                        <div key={index} className='listing-Item'>
                        <div className='listing-Details'>
                            <a href={`/product/${product._id}`}> <div className='Item-image'>
