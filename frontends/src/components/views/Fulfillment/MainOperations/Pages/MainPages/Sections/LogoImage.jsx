...
function LogoImage(props) {
  ...
  return (
    <div style={{ display: 'flex' }}>
        <div className={toggleStage? "Dropzone_hide":"Dropzone_show_logo"} onClick={showImage}>
        <Dropzone
        onDrop={onDrop}
        multiple={false}
        maxSize={800000000}
    >
        {({ getRootProps, getInputProps }) => (
            <div style={{
                width: '100%', cursor: 'pointer', height: '74px',
            ...
            {Images.map((image, index) => (
                <div key={index} onClick={() =>{onDelete(image); showImage()} }>
                    <img style={{  cursor: 'pointer', width: '170px', height: '70px'}} src={`http://localhost:5000/${image}`} alt={`productImg-${index}`} />
                </div>
            ))}
        </div>
