...
export function SearchData(SearchTerms) {
    const request = SearchTerms;
    return {
        type: SEARCH_DATA,
        payload: request
    }
...
export function NavMenuState(PhoneModal) {
    const request = PhoneModal;
    return {
        type: Nav_Menu,
        payload: request
    }
...
export function getMenuState(menuBox) {
    const request = menuBox;
...
export function addToWishlistOnce(_id) {
    const request = axios.get(`${USER_SERVER}/addToWishlistOnce?productId=${_id}`)
        .then(response => response.data);

    return {
        type: ADD_TO_WISHLIST_ONCE_USER,
        payload: request
    }
}
