...
function RunduLocation() {
  ...
  useEffect(() => {
    ...
    const updateDeliveryConfirm = (productId, userId) => {
      ...
      storeAddress: UserData.runduAddress,
    ...
    const RenderStandingOrders = Branch.map((user, index) => {
      ...
      if(user && user.userLocation === "Rundu" ) {
        return (
          <>
            {userInfo.map(info => (
            <>
  ...
  <span className='Branch_title_box_span'>Rundu Orders</span>
    </div>
