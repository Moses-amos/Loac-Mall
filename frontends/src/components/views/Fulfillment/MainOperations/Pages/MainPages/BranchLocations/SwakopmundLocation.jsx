...
useEffect(() => {
  ...
  const updateDeliveryConfirm = (productId, userId) => {
    ...
    storeAddress: UserData.swakopAddress,
  ...
  const RenderStandingOrders = Branch.map((user, index) => {
    ...
    if(user && user.userLocation === "Swakop" ) {
      return (
        <>
          {userInfo.map(info => (
          <>
...
<span className='Branch_title_box_span'>Swakopmund Orders</span>
  </div>
