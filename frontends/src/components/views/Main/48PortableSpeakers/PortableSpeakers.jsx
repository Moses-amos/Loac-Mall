...
import './portablespeaker.css'

function PortableSpeakers() {

  const [Products, setProducts] = useState([])
  const [toggleBrands, settoggleBrands] = useState(0)
  const [UserData, setUserData] = useState([])
  const [Speakers, setSpeakers] = useState([])
  const [SpeakerValue, setSpeakerValue] = useState("Bluetooth Speaker")
...
.then(response => {
  ...
  const SearchedProducts = Products.map((product, index) => {
    ...
    const SpeakersPage = Speakers.map((product, index) => {
      ...
      const JBLSpeakersPage = Speakers.map((product, index) => {
        if (product.seriesName === "JBL") {
        return (
          <div key={index} className='listing-Item'>
          <div className='listing-Details'>
              <a href={`/product/${product._id}`}> <div className='Item-image'>
        ...
        const VolkanoSpeakersPage = Speakers.map((product, index) => {
          if (product.seriesName === "Volkano") {
          return (
            <div key={index} className='listing-Item'>
            <div className='listing-Details'>
                <a href={`/product/${product._id}`}> <div className='Item-image'>
          ...
          const SonySpeakersPage = Speakers.map((product, index) => {
            ...
            const UnlistedSpeakersPage = Speakers.map((product, index) => {
              if (product.seriesName === "Unlisted Speaker") {
              return (
                <div key={index} className='listing-Item'>
                <div className='listing-Details'>
                    <a href={`/product/${product._id}`}> <div className='Item-image'>
...
export default PortableSpeakers