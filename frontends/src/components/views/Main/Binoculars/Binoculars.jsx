...
function Binoculars() {
    ...
    const [BinocularValue, setBinocularValue] = useState("Binocular")

    useEffect(() => {
        const variable = {
          product: BinocularValue
        }
      ...
      if (response.data.success) {
        ...
        setBinoculars(response.data.products)
      } else {
      }
      })
    ...
    .then(response => {
      ...
      const SearchedProducts = Products.map((product, index) => {
          ...
          <div className='Searched_box_listing_container'>
            <a href={`/royal/product/${product._id}`}>
              ...
              <div className='Searched_box_listing_container_right'>
                  ...
                  <a href={`/royal/product/${product._id}`} target="blank" className="Searched_Item_title"><span>{product.title}</span></a>
                  </div>
                  <div style={{ width: '100%', height: '50px', padding: '5px 10px 0px 0px', display: 'block'}}>
                    <div style={{ display: 'flex'}}>
                      ...
                      {product.special !== "" && product.special !== "0" && product.special !== undefined ?
                      <div style={{ paddingTop: '10px'}}>
                          <button className='Searched_SavePercent_style'><span>Save {product.special}</span> </button>
                      </div>
                      :
                      product.specialAttribute !== "" && product.specialAttribute !== "0" && product.specialAttribute !== undefined  ?
                      <div style={{ paddingTop: '10px'}}>
                        <button className='Searched_SavePercent_style'><span>Save {product.specialAttribute}</span> </button>
                      </div>
                      :
                      product.special !== "" && product.specialAttribute !== ""
                      ...
                      <div className='SavePercent'>
                        ...
                        <span className='Percent-Deal_span_overview'>Edit Discount</span>
    </a>
                      </div>
                      :
                      <></>
                    ...
                    <div style={{ display: 'flex', marginTop: '7px'}}>
                      <div className='SavePercent'>
                        <span className='Percent-Deal_span_overview'>{product.warrenty} on Item</span>
                      </div>
                   </div>
                  </div>
                  <div>
        ...
        const BinocularsPage = Binoculars.map((product, index) => {
            return (
              <div key={index} className='listing-Item'>
              <div className='listing-Details'>
                  <a href={`/royal/product/${product._id}`}> <div className='Item-image'>
                ...
                <div className='Item-value'>
                  <div className='Item-price'>
                    ...
                    :
                    product.specialAttribute !== "" && product.specialAttribute !== "0" ?
                    <>
                    <div className='Save-Deals'>
                      <div className='SavePercent'>
                  ...
                  <div className='Save-Deals'>
                  <div className='SavePercent'>
                    <span className='Percent-Deal_span'>Edit Discount</span>
                  </div>
                  {/* <div className='SpecialDeal'>
                        <span>Christmas Special</span>
                      </div>  */}
                ...
                <div className='Warrenty_span-detail_box'>
                  <span className='Warrenty_span-detail'>{product.warrenty} on this Item</span>
                </div>
                <div style={{ paddingTop: '10px'}}>
                {UserData && product === undefined ?
                              ...
                              <div className='Available_In_location-green'>
                              </div>
                              :
                              ...
                              <div className='Available_In_location-blue'>
                              </div>
                              :
                              <div className='Available_In_location-orange'>
                              </div>
                              }
                </div>
                </div>
              ...
              <div className='Company-listing_logo'>
                  {/* <img style={{width: '100%', height: '100%'}}
                   src="https://www.beares.co.za/images/beares-logo.png"
                  /> */}
                  <CompanyImage retailerImage={product.retailerImage} />
                </div>
              </div>
...
getProducts(variables)
  ...
  <div className='Visited_page-banner'>
    <div className='Page_banner-style'>
      ...
      <img src={BackgroundImage} className='Background_Image_style_main'/>
    </div>
    </div>
  </div>
...
<div className="Listed_fetched_products">
  ...
  <div style={{ display: 'flex', padding: '20px 0px 60px 0px'}}>
    <div style={{ minWidth: '280px', height: '490px',marginLeft: '10px', borderRadius: '5px', padding: '10px 15px', border: '1px solid lightgray'}}>
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center'}}>
      <span style={{ fontSize: '22px', fontWeight: '700', color: '#5a596a'}}>Filter Searched</span>
      </div>
      <div style={{ width: '100%', paddingTop: '20px'}}>
      <span style={{ fontSize: '15px', fontWeight: '700', color: '#74767c'}}>* This feature is not yet Available *</span>
      </div>
    </div>
  ...
  :
  SearchTerms === "" || SearchTerms !== "" && searchResults === false ?
  <>
  {Products.length === 0 ?
  <div className='Searching-product_box'>
      {SearchTerms === "" ? 
