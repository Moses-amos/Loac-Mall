...
function PowerCables() {
    ...
    const [LaptopPowerCable, setLaptopPowerCable] = useState([])
    const [DesktopPowerCable, setDesktopPowerCable] = useState([])
    const [DesktopPowerCableValue, setDesktopPowerCableValue] = useState("3 Pin Round Power Cable")
    const [LaptopCableValue, setLaptopCableValue] = useState("Clover Power Cable")
    ...
    useEffect(() => {
        const variable = {
          product: DesktopPowerCableValue
        }
      ...
      if (response.data.success) {
        ...
        setDesktopPowerCable(response.data.products)
      } else {
      }
      })
      ...
      if (response.data.success) {
        ...
        setLaptopPowerCable(response.data.products)
      } else {
      }
      })
      ...
      const SearchedProducts = Products.map((product, index) => {
        ...
        const DesktopPowerCablePage = DesktopPowerCable.map((product, index) => {
            return (
            ...
            const LaptopPowerCablePage = LaptopPowerCable.map((product, index) => {
                return (
                  <div key={index} className='listing-Item'>
                  <div className='listing-Details'>
                      <a href={`/product/${product._id}`}> <div className='Item-image'>
